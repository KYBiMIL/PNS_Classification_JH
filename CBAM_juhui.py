...
def Channel_Attention(FM, reduction_ratio):
...
def Spatial_Attention(F_):
    ...
    concat = concatenate([avr_map, max_map], axis=3)
    # spatial attention
...
def CBAM_block(FM, reduction_ratio=8):
    ...
    F_ = Channel_Attention(FM, reduction_ratio)
    F__ = Spatial_Attention(F_)
...
def identity_block_with_attention(x, f, channel, stage, block):        # skips over 2 layers
...
def JuhuiModelResCBAM(input_shape, num_classes, weights=None):
    ...
    x = identity_block_with_attention(x, f, 64, stage=1, block='a')		# 224
    x = identity_block_with_attention(x, f, 64, stage=1, block='b')
    x = MaxPooling2D(pool_size=(2, 2))(x)                    # 112

    x = identity_block_with_attention(x, f, 128, stage=2, block='a')	
    x = identity_block_with_attention(x, f, 128, stage=2, block='b')
    x = MaxPooling2D(pool_size=(2, 2))(x)                    # 56

    x = identity_block_with_attention(x, f, 256, stage=3, block='a')
    x = identity_block_with_attention(x, f, 256, stage=3, block='b')
    x = Dropout(rate=0.5)(x)
    # DO3 = Dropout(rate=0.5)(C3b)
    x =  MaxPooling2D(pool_size=(2, 2))(x)                   # 50

    x = identity_block_with_attention(x, f, 512, stage=4, block='a')
    x = identity_block_with_attention(x, f, 512, stage=4, block='b')
    x = Dropout(rate=0.5)(x)
    # DO4 = Dropout(rate=0.5)(C4b)
    x =  MaxPooling2D(pool_size=(2, 2))(x)						# 14

    x = identity_block_with_attention(x, f, 1024, stage=5, block='a')
    x = identity_block_with_attention(x, f, 1024, stage=5, block='b')
